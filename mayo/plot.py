...
class Plot(object):
    ...
    def image_format(self):
        return self.config.system.plot.format
                ...
                if self.config.system.plot.parameters:
                    for node, name_value_map in variables.items():
                        for name, value in name_value_map.items():
                            name = '{}-{}'.format(node.formatted_name(), name)
                            name = name.replace('/', '-')
                            # root/{index}/{layer_name}/{variable_name}.{ext}
                            var_path = os.path.join(path, name)
                            self._plot_histogram(value[i], var_path)
    ...
    def _plot_rgb_image(self, value, path):
        ...
        if value.ndim == 3:
            if value.shape[-1] == 1:
                ...
                cmap = None
        path = '{}.{}'.format(path, self.image_format)
